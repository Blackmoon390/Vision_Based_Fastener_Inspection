...
# PROJECT_FOLDER_NAME containing a subfolder BACKEND_FOLDER_NAME which holds
# MAIN_SCRIPT_NAME, CALIBRATION_SCRIPT_NAME and CONFIG_SCRIPT_NAME.
...
def find_project_paths():
    """
    Search every local drive for the project folder, then for the three
    scripts inside its 'backend' subfolder.
    Returns (project_folder, main_path, calibration_path, config_path) as
    ...
    # ---------- Step 1: Find backend folder inside project folder ----------
    for drive in drives:
        try:
            for folder in Path(drive).rglob(PROJECT_FOLDER_NAME):
                if folder.is_dir():
                    backend = folder / BACKEND_FOLDER_NAME
                    if not backend.is_dir():
                        continue
                    calibration = backend / CALIBRATION_SCRIPT_NAME
                    main = backend / MAIN_SCRIPT_NAME
                    config = backend / CONFIG_SCRIPT_NAME
    ...
    # ---------- Step 2: Fallback – find calibration_ui.py anywhere,
    #            confirm main.py is a sibling (both inside backend/) ----------
...
class App:
    ...
    def _fix_paths_done(self, folder, main_p, cal_p, cfg_p=None):
        ...
        backend_folder = os.path.join(str(PROJECT_FOLDER), BACKEND_FOLDER_NAME) if PROJECT_FOLDER else ""
        messagebox.showinfo(
            "Fix Paths",
            f"Project folder:\n{PROJECT_FOLDER}\n\n"
            f"Backend folder:\n{backend_folder}\n\n"
