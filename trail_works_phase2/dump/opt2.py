...
DISPLAY_WIDTH   = 740    # max width for the main display window (height auto-scales)

def make_display(img, max_width=DISPLAY_WIDTH):
    """Resize img so its width ≤ max_width, keeping aspect ratio."""
    h, w = img.shape[:2]
    if w <= max_width:
        return img
    scale = max_width / w
...
while cap.isOpened():
    ...
    if frame_count % PROCESS_EVERY_N != 0:
        cv2.imshow("main", make_display(frame))
    ...
    for result in results:
        ...
        for i, box in enumerate(boxes):
            ...
            if matched:
                ...
                # print("thh:",thh)
                # print("thh2:",totre["thread_height"])
